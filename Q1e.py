...
thetaL2, costL2, maxit2 = batchGradientDescent(X, Y, 0.001)
thetaL3, costL3, maxit3 = batchGradientDescent(X, Y, 0.025)
thetaL4, costL4, maxit4 = batchGradientDescent(X, Y, 0.1)
...
def plotContour(thetaL, costL, eta):
	...
	for itr in range(len(thetaL)):
		plt.plot(thetaL[itr][0], thetaL[itr][1], color='orange', marker='.')
		#plt.pause(0.001)

	f = "Q1eContour" + str(eta) + ".png"
	contour = os.path.join(out_dir, f)
	plt.savefig(contour)
	plt.close()

plotContour(thetaL2, costL2, 0.001)
plotContour(thetaL3, costL3, 0.025)
plotContour(thetaL4, costL4, 0.1)